...
export default function TeacherRegisterPage() {
  ...
  useEffect(() => {
    console.log('👨‍🏫 [Teacher Register Page] Teacher register page loaded');
  }, []);
  ...
  const handleSubmit = async () => {
    ...
    console.log('🔍 [Teacher Register] بدء عملية التسجيل...', {
    ...
    try {
      ...
      const apiUrl = getApiUrl('/teachers/register/');
      console.log('📡 [Teacher Register] Sending request to:', apiUrl);
      
      const response = await fetch(apiUrl, { method: 'POST', body: formData });
      
      console.log('📡 [Teacher Register] Response received:', {
      ...
      if (!response.ok) {
        ...
        if (data.errors) {
          ...
          console.error('❌ [Teacher Register] Validation errors:', {
            errors: data.errors,
            errorMessages,
            timestamp: new Date().toISOString()
          });
        ...
        const errorMessage = data.detail || data.message || 'فشل في التسجيل';
        console.error('❌ [Teacher Register] Registration failed:', {
          status: response.status,
          error: errorMessage,
          data: data,
          timestamp: new Date().toISOString()
      ...
      console.log('✅ [Teacher Register] Registration successful:', {
      ...
      console.error('❌ [Teacher Register] Registration error:', {
        error: errorMessage,
    ...
    <div dir="rtl" className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 py-10 px-4 pt-24">
      ...
      <div className="max-w-4xl mx-auto relative z-10">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-8">
          <Link href="/" className="inline-flex items-center gap-2 text-[#1e40af] hover:text-[#1e3a8a] dark:text-blue-400 dark:hover:text-blue-400 mb-4">
            <Home size={20} /><span>العودة للرئيسية</span>
          </Link>
          <div className="flex justify-center mb-4">
            <Image src="/logo.png" alt="Noor Academy Logo" width={80} height={80} className="rounded-2xl shadow-lg" />
          </div>
          <h1 className="text-3xl md:text-4xl font-bold bg-gradient-to-r from-[#1e40af] to-[#2563eb] dark:from-blue-400 dark:to-[#3b82f6] bg-clip-text text-transparent mb-2">
            انضم كمعلم في أكاديمية نور
          </h1>
          ...
          {[1, 2].map((step) => (
            <div key={step} className="flex items-center">
              <div className={`flex items-center justify-center w-10 h-10 rounded-full font-bold transition-all ${
                currentStep >= step ? 'bg-[#1e40af] dark:bg-blue-500 text-white shadow-lg' : 'bg-slate-200 dark:bg-slate-700 text-slate-500 dark:text-slate-400'
              }`}>{currentStep > step ? <CheckCircle size={20} /> : step}</div>
              {step < 2 && <div className={`w-16 h-1 mx-2 transition-all ${currentStep > step ? 'bg-[#1e40af] dark:bg-blue-500' : 'bg-slate-200 dark:bg-slate-700'}`} />}
            </div>
          ))}
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }} className="text-center mt-6 text-slate-600 dark:text-slate-400">
          <p>لديك حساب بالفعل؟ <Link href="/login" className="text-[#1e40af] hover:text-[#1e3a8a] dark:text-blue-400 dark:hover:text-blue-400 font-semibold">تسجيل الدخول</Link></p>
        </motion.div>
      </div>
    </div>
  );
