    ...
    <div dir="rtl" className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 py-10 px-4 pt-24">
      ...
      <div className="max-w-4xl mx-auto relative z-10">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-8">
          <Link href="/" className="inline-flex items-center gap-2 text-[#0A5734] hover:text-[#073D24] dark:text-[#4A8F5C] dark:hover:text-[#3A7148] mb-4">
          ...
          <h1 className="text-3xl md:text-4xl font-bold bg-gradient-to-r from-[#0A5734] to-[#4A8F5C] dark:from-[#4A8F5C] dark:to-[#5BA86D] bg-clip-text text-transparent mb-2">
          ...
          {[1, 2].map((step) => (
            <div key={step} className="flex items-center">
              <div className={`flex items-center justify-center w-10 h-10 rounded-full font-bold transition-all ${
                currentStep >= step ? 'bg-[#0A5734] dark:bg-[#4A8F5C] text-white shadow-lg' : 'bg-slate-200 dark:bg-slate-700 text-slate-500 dark:text-slate-400'
              }`}>{currentStep > step ? <CheckCircle size={20} /> : step}</div>
              {step < 2 && <div className={`w-16 h-1 mx-2 transition-all ${currentStep > step ? 'bg-[#0A5734] dark:bg-[#4A8F5C]' : 'bg-slate-200 dark:bg-slate-700'}`} />}
            </div>
          ))}
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }} className="text-center mt-6 text-slate-600 dark:text-slate-400">
          <p>لديك حساب بالفعل؟ <Link href="/login" className="text-[#0A5734] hover:text-[#073D24] dark:text-[#4A8F5C] dark:hover:text-[#3A7148] font-semibold">تسجيل الدخول</Link></p>
        </motion.div>
      </div>
    </div>
  );
...
function Step1Form({ data, onChange, onNext, showPassword, setShowPassword, showPassword2, setShowPassword2 }: any) {
  return (
    <motion.div key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
      ...
      <button type="button" onClick={onNext} className="w-full bg-gradient-to-r from-[#0A5734] to-[#4A8F5C] hover:from-[#073D24] hover:to-[#3A7148] text-white font-bold py-4 px-6 rounded-lg shadow-lg hover:shadow-xl transition-all flex items-center justify-center gap-2"><span>التالي</span><ArrowRight size={20} /></button>
    </motion.div>
  );
}
...
function Step2IslamicForm({ data, onChange, onBack, onSubmit, isSubmitting }: any) {
  return (
    <motion.div key="step2-islamic" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
      ...
      <div><label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-3"><Award className="inline ml-2" size={16} />هل لديك إجازة في القرآن الكريم؟ *</label><div className="flex gap-4"><label className="flex items-center gap-2 cursor-pointer"><input type="radio" name="has_ijazah" checked={data.has_ijazah === true} onChange={() => onChange({ ...data, has_ijazah: true })} className="w-4 h-4 text-[#0A5734] dark:text-[#4A8F5C] focus:ring-[#0A5734] dark:focus:ring-[#4A8F5C]" /><span className="text-slate-700 dark:text-slate-300">نعم</span></label><label className="flex items-center gap-2 cursor-pointer"><input type="radio" name="has_ijazah" checked={data.has_ijazah === false} onChange={() => onChange({ ...data, has_ijazah: false })} className="w-4 h-4 text-[#0A5734] dark:text-[#4A8F5C] focus:ring-[#0A5734] dark:focus:ring-[#4A8F5C]" /><span className="text-slate-700 dark:text-slate-300">لا</span></label></div></div>
