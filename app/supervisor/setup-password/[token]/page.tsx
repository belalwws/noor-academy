...
export default function SupervisorPasswordSetupPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.detail || data.message || 'فشل في التسجيل');
  ...
  useEffect(() => {
    async function validate() {
      ...
      if (hasValidated.current) {
        console.log('⚠️ Token validation already performed, skipping...');
        return;
      }
      ...
      try {
        ...
        if (data.success && data.data?.valid) {
          ...
          setError('رابط الدعوة غير صالح أو قد انتهت صلاحيته');
          console.log('❌ Token is invalid');
        }
      } catch (err) {
        setError('تعذر التحقق من الرابط');
    ...
    return (
      <div dir="rtl" className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-600 dark:text-slate-400">جاري التحقق...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div dir="rtl" className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 flex items-center justify-center px-4">
        <div className="max-w-md w-full bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 p-8">
          ...
          <h2 className="text-2xl font-bold text-center text-slate-900 dark:text-white mb-4">رابط غير صالح</h2>
      ...
      <div className="max-w-4xl mx-auto relative z-10">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-8">
          ...
          <h1 className="text-3xl md:text-4xl font-bold bg-gradient-to-r from-amber-900 to-orange-900 dark:from-amber-300 dark:to-orange-300 bg-clip-text text-transparent mb-2">
            إنشاء حساب مشرف - أكاديمية رُشد
          </h1>
          <p className="text-slate-600 dark:text-slate-400">أكمل بياناتك للبدء في الإشراف على المعلمين</p>
          {email && (
            <div className="mt-4 inline-flex items-center gap-2 px-4 py-2 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg">
              ...
              <span className="text-sm font-medium text-amber-900 dark:text-amber-200">
                البريد الإلكتروني: <span className="font-bold">{email}</span>
              </span>
            </div>
          )}
        </motion.div>
  ...
  return (
    <motion.div key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="relative">
          <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">
            <Lock className="inline ml-2" size={16} />تأكيد كلمة المرور *
          </label>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">
            <Calendar className="inline ml-2" size={16} />العمر *
          </label>
        ...
        <div>
          <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">
            <User className="inline ml-2" size={16} />الجنس *
          </label>
      ...
      <div>
        <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">
          <Phone className="inline ml-2" size={16} />رقم الهاتف *
        </label>
      ...
      <div>
        <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">
          <GraduationCap className="inline ml-2" size={16} />المؤهل الأكاديمي *
        </label>
      ...
      <div>
        <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">
          <BookOpen className="inline ml-2" size={16} />التخصص *
        </label>
        <select name="specialization" value={data.specialization} onChange={onChange} className="w-full px-4 py-3 rounded-lg border-2 border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white focus:border-amber-500 focus:ring-2 focus:ring-amber-200 dark:focus:ring-amber-900 transition-all" required>
          ...
          <option value="quran_islamic">قرآن وعلوم شرعية</option>
          <option value="other">أخرى</option>
        </select>
      </div>
      {data.specialization === 'other' && (
        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}>
          <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">حدد التخصص *</label>
    ...
    <motion.div key="step2-islamic" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
      ...
      <div>
        <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-3">
          <Award className="inline ml-2" size={16} />هل لديك إجازة في القرآن الكريم؟ *
        </label>
        <div className="flex gap-4">
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            <span className="text-slate-700 dark:text-slate-300">نعم</span>
          </label>
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            <span className="text-slate-700 dark:text-slate-300">لا</span>
          </label>
        </div>
      </div>
      {data.has_ijazah && (
        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}>
          <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">جهة الإجازة *</label>
      ...
      <div>
        <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">
          <BookOpen className="inline ml-2" size={16} />التخصص الشرعي الدقيق
        </label>
        <select value={data.islamic_specialization} onChange={(e) => onChange({ ...data, islamic_specialization: e.target.value })} className="w-full px-4 py-3 rounded-lg border-2 border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white focus:border-amber-500 focus:ring-2 focus:ring-amber-200 dark:focus:ring-amber-900 transition-all">
          ...
          <option value="tafseer">تفسير</option>
...
function Step2OtherForm({ data, onChange, onBack, onSubmit, isSubmitting }: any) {
  ...
  return (
    <motion.div key="step2-other" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
      ...
      <div>
        <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">
          <GraduationCap className="inline ml-2" size={16} />اسم الجامعة
        </label>
        <input type="text" value={data.university_name} onChange={(e) => onChange({ ...data, university_name: e.target.value })} className="w-full px-4 py-3 rounded-lg border-2 border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white focus:border-amber-500 focus:ring-2 focus:ring-amber-200 dark:focus:ring-amber-900 transition-all" placeholder="جامعة القاهرة" />
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">
            <Calendar className="inline ml-2" size={16} />سنة التخرج
          </label>
        ...
        <div>
          <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">
            <Briefcase className="inline ml-2" size={16} />سنوات الخبرة
          </label>
      ...
      <div>
        <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">
          <FileText className="inline ml-2" size={16} />شهادة التخرج (اختياري)
        </label>
      ...
      <div>
        <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">
          <FileText className="inline ml-2" size={16} />السيرة الذاتية CV (اختياري)
        </label>
      ...
      <div>
        <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">
          <Globe className="inline ml-2" size={16} />رابط LinkedIn (اختياري)
        </label>
