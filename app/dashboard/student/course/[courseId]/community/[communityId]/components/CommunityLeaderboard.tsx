        ...
        {leaderboard.length === 0 ? (
          ...
          <div className="space-y-4">
            {leaderboard.map((entry) => (
              <Card
                key={entry.member.id}
                className={`border-2 transition-all duration-300 ${getRankColor(entry.rank)}`}
              >
                <CardContent className="p-4">
                  <div className="flex items-center gap-4">
                    ...
                    {/* Avatar */}
                    <Avatar className="w-12 h-12 ring-2 ring-white dark:ring-slate-700">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-4 text-sm text-gray-600 dark:text-gray-400">
                        ...
                        <span className="font-semibold text-amber-600 dark:text-amber-400">
                          {entry.member.total_points} نقطة
                        </span>
                      </div>
                    </div>
                    ...
                    {entry.badges && entry.badges.length > 0 && (
                      <div className="flex items-center gap-2 flex-shrink-0">
                        {entry.badges.slice(0, 5).map((badge) => (
                          <div
                            key={badge.id}
                            className="w-8 h-8 rounded-full bg-gradient-to-br from-amber-400 to-orange-500 flex items-center justify-center text-white text-xs"
                            title={badge.name}
                          >
