...
interface CreateUnitExamFormProps {
  labId: string;
...
export function CreateUnitExamForm({ labId, courseId, onSuccess, onCancel }: CreateUnitExamFormProps) {
  ...
  const getContentTypeId = async (modelName: string): Promise<number> => {
    // First, try to get ContentType ID from existing unit exams
    try {
      const token = localStorage.getItem('access_token');
      const response = await fetch(
        `${API_BASE_URL}/knowledge-lab/unit-exams/?knowledge_lab=${labId}&limit=1`,
        {
          headers: {
            'Authorization': `Bearer ${token}`,
          },
      ...
      if (response.ok) {
        ...
        if (data.results && data.results.length > 0) {
          ...
          if (existingExam.unit_content_type) {
            console.log(`✅ Found ContentType ID from existing exam: ${existingExam.unit_content_type}`);
            return existingExam.unit_content_type;
          }
        }
      }
    } catch (error) {
  ...
  const handleUnitSelect = (unitId: string) => {
    ...
    if (selectedUnit) {
      // Use the ContentType ID from the unit (which comes from API response)
      const correctContentTypeId = selectedUnit.contentTypeId;
      ...
      console.log(`✅ Using ContentType ID from unit: ${correctContentTypeId}`);
      
      setFormData({
        ...formData,
        unit_object_id: selectedUnit.id,
        unit_content_type: correctContentTypeId,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Use the ContentType ID from formData (which was set by handleUnitSelect)
      const contentTypeId = formData.unit_content_type > 0 
        ? formData.unit_content_type 
        : selectedUnit.contentTypeId;

      if (contentTypeId === 0 || !contentTypeId) {
        toast.error('خطأ في نوع المحتوى. يرجى اختيار الوحدة مرة أخرى');
        setLoading(false);
        return;
      }
      ...
      const payload = {
        knowledge_lab: labId,
        unit_content_type: contentTypeId,
      ...
      console.log('📤 Creating unit exam with payload:', payload);

      const response = await knowledgeLabApi.createUnitExam(payload);

      if (response.success) {
        toast.success('تم إنشاء الاختبار بنجاح! ✨');
        onSuccess();
      } else {
        console.error('❌ Failed to create unit exam:', response);
        const errorMsg = response.error || response.errors?.detail || 'فشل في إنشاء الاختبار';
        toast.error(errorMsg);
      }
    } catch (error) {
      console.error('❌ Error creating unit exam:', error);
    ...
    <Card className="border-orange-200 shadow-lg" dir="rtl">
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="text-xl">إنشاء اختبار وحدة جديد</CardTitle>
            <CardDescription>اختر الأسئلة من بنك الأسئلة وأنشئ اختبار وحدة</CardDescription>
          </div>
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex gap-3 justify-end">
            ...
            <Button
              type="submit"
              disabled={loading || selectedQuestions.length === 0 || !formData.unit_object_id}
