...
import { BookOpen, Plus, Trash2, Clock, FileQuestion, Eye } from 'lucide-react';
import { toast } from 'sonner';
import { useRouter } from 'next/navigation';
import { knowledgeLabApi } from '@/lib/api/knowledge-lab';
import type { LessonExercise } from '@/lib/api/knowledge-lab';
import { CreateLessonExerciseForm } from './CreateLessonExerciseForm';
...
export function LessonExercisesTab({ labId, courseId }: LessonExercisesTabProps) {
  ...
  const fetchExercises = async () => {
    setLoading(true);
    try {
      const response = await knowledgeLabApi.listLessonExercises({
        knowledge_lab: labId,
      });
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const response = await knowledgeLabApi.deleteLessonExercise(id);
      if (response.success) {
  ...
  return (
    <div className="space-y-4" dir="rtl">
      ...
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold">تمارين الدروس</h3>
        <Button 
      ...
      {exercises.length === 0 ? (
        <Card className="border-orange-100" dir="rtl">
          <CardContent className="py-12 text-center text-gray-500">
            ...
            <p className="text-lg font-semibold mb-2">لا توجد تمارين للدروس</p>
            <p className="text-sm">أنشئ تمرين درس جديد من بنك الأسئلة</p>
          </CardContent>
        </Card>
      ) : (
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    {exercise.lesson_title && (
                      <CardDescription className="mb-2">الدرس: {exercise.lesson_title}</CardDescription>
                    )}
                  ...
                  <div className="flex gap-2">
                    <Button
                      ...
                      variant="outline"
                      onClick={() => {
                        router.push(`/knowledge-lab/${labId}/exercises/${exercise.id}?type=lesson-exercise`);
                      }}
